const fileContent = `# Apache Software License 2.0
#
...
def inference_preprocessor(
    ...
    dataset_inf = preprocess_pipeline.transorm(dataset_inf)
    dataset_inf.drop(columns=[target], inplace=True)
    return dataset_inf
`;

const oldContent = `@step
def inference_preprocessor(
    dataset_inf: pd.DataFrame,
    preprocess_pipeline: Pipeline,
    target: str,
...
const findFirstLineNumber = (str, substr) => {
